...
c = c.replace(
  new RegExp('<button\\\\s\\+onClick=\\\\{\\\\(\\\\) => navigate\\\\(\\'\\\\/generador-flyer\\'\\\\)\\\\\}\\\\s\\+className="btn-secondary"[\\\\s\\\\S]*?<\\\\/button>'),
...
c = c.replace(
  new RegExp('<button onClick=\\\\{\\\\(\\\\) => navigate\\\\(\\'\\\\/generador-flyer\\'\\\\)\\\\\} className="btn-primary"[\\\\s\\\\S]*?<\\\\/button>'),
  {(currentUser?.isSuperAdmin || currentUser?.appRole === 'gerente' || currentUser?.isDireccion || currentUser?.appRole === 'director_maestria' || currentUser?.appRole?.includes('coord')) && (
