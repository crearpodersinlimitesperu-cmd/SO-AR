...
const target1 = "      <TaskAssignmentModal isOpen={showTaskForm} onClose={() => setShowTaskForm(false)} />\r\n      <VenueConfigModal isOpen={showVenueModal} onClose={() => setShowVenueModal(false)} />";
const target2 = "      <TaskAssignmentModal isOpen={showTaskForm} onClose={() => setShowTaskForm(false)} />\n      <VenueConfigModal isOpen={showVenueModal} onClose={() => setShowVenueModal(false)} />";

let updatedContent = content;
if (updatedContent.includes(target1)) {
    updatedContent = updatedContent.replace(target1, target1 + "\n" + horarioCode);
} else if (updatedContent.includes(target2)) {
    updatedContent = updatedContent.replace(target2, target2 + "\n" + horarioCode);
} else {
    console.log("No se encontro el lugar para inyectar el horario");
}
