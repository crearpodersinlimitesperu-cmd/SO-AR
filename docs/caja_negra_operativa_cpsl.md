...
| **9** | **Gobernanza y Acuse Logístico de Horarios** | `src/services/scheduleGovernanceAgent.js` | Enforce de jerarquía (solo Gerentes/SuperAdmin editan; Coordinadores solo lectura). Despacho in-app en Causa OS, acuse de recibo vinculante ("Leído y Enterado") y auditoría de sala. |

### 3.1. Protocolo de Gobernanza y Acuse de Recibo Logístico (`ScheduleGovernanceAgent`)
* **Principios de Gerencia Organizacional & Logística:**
  1. **Principio de Autoridad & Mando:** Solo los Gerentes de Sede (`SCHEDULE_SEDE_MANAGERS`), Dirección General y SuperAdmins (`GLOBAL_SUPER_ADMINS`) tienen potestad para crear, editar, reasignar o eliminar turnos y vestimenta. Los Coordinadores (CC1, CC2, CMJ) acceden a un **Modo Consulta Oficial** blindado.
  2. **Notificación Proactiva en Causa OS:** Cada actualización de la Gerencia genera notificaciones inmediatas en la bandeja interna de Causa OS (`collection(db, 'notifications')`) para todo el equipo asignado en la sede (`userId: email`), asegurando que ningún colaborador quede desinformado.
  3. **Acuse de Recibo Vinculante ("Leído y Enterado"):** Sello digital con fecha, hora, rol y usuario autenticado guardado en Firestore (`nodus_training_schedules.{sede}_horarios_equipos.acknowledgements`), certificando la aceptación de la guardia operativa de sala.
  4. **Auditoría Logística en Vivo:** Módulo de monitoreo para la Gerencia con tasa de cumplimiento porcentual, lista de colaboradores confirmados y semáforo de pendientes.
  5. **Despacho Multi-Canal con Copias Obligatorias:**
