...
(async () => {
  ...
  try {
    ...
    // Scrape data from /participantes datatable via the API endpoint
    const data = await page.evaluate(async () => {
        // The datatable usually hits an endpoint like /participantes/datosTabla
        const res = await fetch('https://imo.crearpslglobal.com/participantes/datosTabla?draw=1&start=0&length=100&search[value]=&order[0][column]=0&order[0][dir]=asc');
        return res.json();
    });
    
    // Count how many from each sede
    const sedes = {};
    if (data && data.data) {
        data.data.forEach(row => {
            const sede = row.sede || row[4] || 'Desconocida';
