...
function GlobalView({ tasks, navigate }) {
  ...
  const handleSyncUsers = async () => {
    if (!window.confirm('¿Inicializar y sincronizar la base de datos completa en Firebase? (Inyectará 63 colaboradores, sedes y metadatos)')) return;
    try {
      showToast('Iniciando carga de base de datos en Firebase...', 'info');
      const results = await initializeDatabase((msg) => showToast(msg, 'info'));
      showToast(`✅ Base de datos inicializada: ${results.usersCreated} usuarios y ${results.venuesCreated} sedes listas en Firestore.`, 'success');
    } catch (e) {
      console.error(e);
  ...
  const roleGroups = [
    ...
    { id: 'coord_c1', alias: 'coordinador_c1c2', label: 'Coordinadores C1/C2' },
    { id: 'coord_maestria', alias: 'coordinador_mj', label: 'Coordinadores MJ' },
...
function RoleView({ tasks, navigate, onSelectUser, onAssignTask }) {
  const roles = [
    ...
    { id: 'coord_c1', label: 'Coordinadores C1/C2' },
    { id: 'coord_maestria', label: 'Coordinadores de Maestría' },
