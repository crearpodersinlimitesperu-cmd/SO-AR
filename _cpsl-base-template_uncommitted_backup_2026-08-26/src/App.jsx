...
function RoleRoute({ children, allowedRoles = [], requireSuperAdmin = false }) {
  ...
  if (allowedRoles.length > 0) {
    const hasRole = 
      ...
      (currentUser.isDireccion && (allowedRoles.includes('direccion') || allowedRoles.includes('gerente') || allowedRoles.includes('director_maestria'))) ||
      (currentUser.isGerente && allowedRoles.includes('gerente')) ||
      allowedRoles.includes(currentUser.appRole) ||
      (currentUser.roles && currentUser.roles.some(r => allowedRoles.includes(r)));
